...
public class inputfile_type {
	...
	public static void main(String[] args) throws IOException {
		...
		FileReader fwin = new FileReader("./input.txt");	// input.txt 파일 생성 & 오픈
		...
		try{
			...
			if(n<1 || n>100){
				...
				bufw.mark(1000);	// buffer reset point marking
				while((bufw.readLine()) != null) { previouschk++; }
				if(n!=previouschk) {
					System.out.println(">> " + n + "개의 트랜잭션이 아닙니다.");
					tr.close();
					fwin.close();
					fwout.close();
					bufw.close();
			...
			while(true){
				if((str = bufw.readLine()) == null) { break; }
